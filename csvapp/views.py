...
class FuneralPolicyList(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            updated_records = []
            failed_records = []
            repeating_record_instances = []

            for row in worksheet.iter_rows(values_only=True):

                policy_number = row[10]
                existing_record = FuneralPolicy.objects.filter(policy_number=policy_number).first()

                if existing_record:
                    for field, value in zip(FuneralPolicy._meta.fields, row):
                        setattr(existing_record, field.name, value)
                    existing_record.save()
                    updated_records.append(FuneralPolicySerializer(existing_record).data)
                ...
                try:
                    ...
                    funeral_policy.save()
                    new_records.append(FuneralPolicySerializer(funeral_policy).data)

                except IntegrityError:
                   failed_records.append(FuneralPolicySerializer(funeral_policy).data)

            total_records = FuneralPolicy.objects.all().count()

            return Response({
                'message': 'Funeral policy data CSV file uploaded successfully',
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            for row in worksheet.iter_rows(values_only=True):
                ...
                existing_record = IndluLoanData.objects.filter(loan_ref_Id=loan_ref_Id).first()

                if existing_record:
                    for field, value in zip(IndluLoanData._meta.fields, row):
                        setattr(existing_record, field.name, value)
                    existing_record.save()
                    updated_records.append(IndluLoanDataSerializer(existing_record).data)
                else:
                    report_data = row[0]
                    client_ref = row[1]
                    loan_ref_Id = row[2]
                ...
                try:
                    ...
                    new_records.append(IndluLoanDataSerializer(indlu_data_instance).data)

                except IntegrityError:
                    failed_records.append(IndluLoanDataSerializer(indlu_data_instance).data)
            ...
            return Response({
                ...
                'updated_records': updated_records,
                'failed_records': failed_records,
                'repeating_record_instances': repeating_record_instances
            }, status=201)               
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            for row in worksheet.iter_rows(values_only=True):
                ...
                refId = row[0]
                existing_record = SmartAdvanceCredit.objects.filter(refId=refId).first()

                if existing_record:
                    for field, value in zip(SmartAdvanceCredit._meta.fields, row):
                        setattr(existing_record, field.name, value)
                    existing_record.save()
                    updated_records.append(SmartAdvanceCreditSerializer(existing_record).data)
                ...
                try:
                    ...
                    new_records.append(SmartAdvanceCreditSerializer(smart_advance_credit_instance).data)
                except IntegrityError:
                    failed_records.append(SmartAdvanceCreditSerializer(smart_advance_credit_instance).data)

            total_records = SmartAdvanceCredit.objects.all().count()

            return Response({
                'message':'Smart Advance Credit Info CSV uploaded successfully',
                'total_records':total_records,
                'new_records': new_records,
                'updated_records': updated_records,
                'failed_records': failed_records,
